...
def plot_var_cor(x: Union[pd.DataFrame, np.ndarray], ax=None, return_values: bool = False, **kwargs) -> Optional[
    np.ndarray]:
    """
...
def plot_correlation_difference(real: pd.DataFrame, fake: pd.DataFrame, plot_diff: bool = True, cat_cols: list = None,
                                name: str = None, save_path: str = None, annot=False):
    """
    ...
    plt.tight_layout()
    plt.savefig(save_path + "/" + name + "/correlation_difference.png")
    plt.show()
...
def plot_correlation_comparison(evaluators: List, annot=False):
    ...
    fake_corr = []
    real_corr = \
    associations(evaluators[0].real, nominal_columns=evaluators[0].categorical_columns, plot=False, theil_u=True,
    ...
    for i in range(1, nr_plots):
        ...
        fake_corr.append(
            associations(evaluators[i - 1].fake, nominal_columns=evaluators[0].categorical_columns, plot=False,
                         theil_u=True,
...
def plot_mean_std(real: pd.DataFrame, fake: pd.DataFrame, name, save_path, ax=None):
    """
    ...
    plt.savefig(save_path + "/" + name + "/mean_std.png")
    plt.show()
